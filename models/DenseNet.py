...
class _DenseLayer(nn.Module):
    ...
    @torch.jit.unused
    def call_checkpoint_bottlenect(self, input: List[Tensor]) -> Tensor:
        def closure(*inputs):
    ...
    def forward(self, input: Tensor) -> Tensor: 
        ...
        if self.memory_efficient and self.any_requires_grad(prev_features):
            ...
            bottleneck_output = self.call_checkpoint_bottlenect(prev_features)
...
class _DenseBlock(nn.ModuleDict):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            layer = _DenseLayer(num_input_feature + i * growth_rate, 
...
def densenet121(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> DenseNet:
    return _densenet("densenet121", 32, (6, 12, 24, 16), 16, pretrained, progress, **kwargs)
